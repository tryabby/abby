  ...
  return (
    ...
    >
      <div>
        ...
        <h2 className={cn('mb-3 mt-6 text-xl font-semibold', isFull && 'lg:my-3')}>{title}</h2>
        {/* <p className={cn("te h-32", isFull && "lg:h-auto")}>{subtitle}</p> */}
      </div>
      <div
        className={cn(
          'flex flex-col',
          isFull && 'lg:col-span-2 lg:ml-8 lg:grid lg:grid-cols-2 lg:items-center',
        )}
      >
        <Link
          href={href}
          ...
          onClick={() => trackEvent('Plan Selected', { props: { Plan: planName } })}
        >
          ...
          {features.map((feature, i) => {
            return (
              <Fragment key={feature}>
                ...
                <span className={clsx('flex items-center', i === 0 ? 'font-semibold' : 'te')}>
                  {feature}
                  {i === 0 && (
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p className='max-w-xs text-sm'>
                          This number includes the events that are used for A/B testing analytics as
                          well as the amount of requests to our servers.
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  )}
    ...
    <div>
      <div className='my-6 flex items-center justify-center'>
        <div className='flex items-center space-x-2'>
          ...
          <Switch id='currency' checked={useEuro} onCheckedChange={setUseEuro} />
