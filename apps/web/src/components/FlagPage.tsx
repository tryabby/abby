...
const EditTitleModal = ({
  ...
  return (
    ...
    >
      <Input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder='MyFlag' />
    </Modal>
  )
}
...
const EditDescriptionModal = ({
  ...
  const trpcContext = trpc.useContext()
  const { mutate: updateDescription } = trpc.flags.updateDescription.useMutation({
    onSuccess() {
  ...
  return (
    <Modal
      ...
      confirmText='Save'
      onConfirm={() => updateDescription({ flagId, description: newDescription })}
      size='full'
      isOpen={isOpen}
      onClose={onClose}
    >
      <Editor className='w-full' content={newDescription} onUpdate={setNewDescription} />
    </Modal>
  )
}
...
export const FeatureFlagPageContent = ({
  ...
  data: NonNullable<InferQueryResult<(typeof appRouter)['flags']['getFlags']>['data']>
    ...
    <>
      <div>
        <div className='flex justify-end space-x-2'>
          ...
          >
            <AiOutlinePlus /> <span>Add {type === 'Flags' ? 'Flag' : 'Config'}</span>
          </Button>
          <AddFeatureFlagModal
        ...
        <div className='space-y-3'>
          {data.flags.map((currentFlag) => {
            return (
              <section key={currentFlag.id} className='w-full rounded-md bg-secondary p-4'>
                <div className='flex justify-between'>
                  <div className='flex items-center space-x-3'>
                    <h2 className='font-bold text-pink-200'>{currentFlag.name}</h2>
                      ...
                      >
                        ...
                        <p
                          ...
                          dangerouslySetInnerHTML={{
                            __html: currentFlag.description ?? '<p>No description</p>',
                          }}
                        />
                      </TooltipContent>
                    </Tooltip>
                ...
                <div className='relative mt-3 grid grid-cols-[repeat(auto-fill,minmax(205px,1fr))] gap-x-8 gap-y-4 pr-2'>
                  {currentFlag.values
                    .sort((a, b) => a.environment.sortIndex - b.environment.sortIndex)
                    .map((flagValue) => (
                      <FeatureFlag
