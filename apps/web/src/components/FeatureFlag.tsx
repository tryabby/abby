  ...
  const { mutate: updateFlag } = trpc.flags.updateFlag.useMutation({
    onSuccess(_, { value, flagValueId }) {
      trpcContext.flags.getFlags.setData({ projectId, types: [] }, (prev) => {
        ...
        const valueToUpdate = flagToUpdate.values.find((value) => value.id === flagValueId)
...
export function FeatureFlag({ flag, projectId, environmentName, flagValueId, type }: Props) {
