...
export function CreateTestSection({ setTestName, testName, setVariants, variants }: Props) {
  ...
  const weightSum = Math.round(
    variants.map(({ weight }) => weight).reduce((sum, weight) => (sum += weight), 0)
  )
  ...
  const handleWeightChange = (index: number, event: ChangeEvent<HTMLInputElement>) => {
    ...
    const eventValue = typeof rawEventValue === 'number' ? rawEventValue : parseInt(rawEventValue)
              ...
              return (
                <Fragment key={i}>
                  ...
                  >
                    ...
                    {variants.filter((variant) => variant.name === name).length > 1 && (
            ...
            <div className='grid grid-cols-[1fr_auto_1fr]'>
              ...
              {weightSum !== 100 ? (
                <p className='text-center text-ab_accent-background'>
                  Your weights must add up to 100%. Your weights currently make up {weightSum}%
                </p>
              ) : (
                <span></span>
              )}
