import { updateConfigOnServer } from "./http";
import { getConfigFromFileString, loadLocalConfig } from "./util";




export async function push(filePath: string, apiKey: string, localhost?: boolean): Promise<void> {
  ...
  await updateConfigOnServer(projectId, apiKey, localAbbyConfig, localhost);
}
