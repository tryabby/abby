...
// Constants
const NUM_ENTRIES = 10_000_000 / 10000; // Number of entries you want to insert
const PROJECT_COUNT = 100;
const API_VERSION = "V0";
const TYPES = ["GET_CONFIG", "POST_UPDATE", "DELETE_RECORD", "PUT_RESOURCE"];
const BATCH_SIZE = 10000 / 10; // Size of each batch
...
async function insertBatch(records: any[]) {
  const query = `
    ...
    VALUES ${records.map((record) => `('${record.id}', '${record.createdAt}', '${record.type}', ${record.durationInMs}, '${record.apiVersion}', '${record.projectId}')`).join(",")}
  `;
  try {
...
async function generateAndInsertRecords() {
  const records: any[] = [];
  let batchCount = 0;
  ...
  for (let i = 0; i < NUM_ENTRIES; i++) {
    ...
    if (records.length >= BATCH_SIZE) {
      console.log(`Inserting batch ${++batchCount}`);
  ...
  if (records.length > 0) {
    console.log(`Inserting final batch ${++batchCount}`);
    await insertBatch(records);
  }
...
generateAndInsertRecords().catch((err) => {
